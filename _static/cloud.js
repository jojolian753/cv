...
$(document).ready(function (){
  ...
  var toc_header = $('.sphinxsidebar p.logo + h3');
  var toc_adjust = toc_header.length ? toc_header.offset().top - holder.offset().top - 8 : 0;
  var toggle = $('.sidebartoggle');

  // function to set style for given state
  function set_style(target, value, adjust)
  {
    if(value <= adjust || disabled){
      target.css({marginLeft: "", position: "", top: "", left: "", bottom: ""});
    }
    else if (value <= holder.height() - target.height() + adjust){
      target.css({marginLeft: 0, position: "fixed", top: -adjust, left: holder.offset().left, bottom: ""});
    }
    else{
  ...
  function update_sticky(){
    ...
    var leftover = $(window).height() - sidebar.height();
    disabled = (leftover < 0);
    if(disabled && leftover + toc_adjust >= 0){
      // not enough room for full sidebar, but there is enough starting w/ toc
      disabled = false;
      set_style(sidebar, offset, toc_adjust);
    }else{
      // enough room for full sidebar
      set_style(sidebar, offset, 0);
    }
